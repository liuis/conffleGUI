...
export const getReceipts = async function(transactions, web3Instance) {
  return await Promise.all(
    transactions.map(tx => {
      return web3Request("cfx_getTransactionReceipt", [tx.hash], web3Instance);
    }),
  );
};
...
export const getTransactionsForBlocks = function(
  ...
  return async function(dispatch, getState) {
    ...
    for (let number = endBlockNumber; number >= startBlockNumber; number--) {
      ...
      let block = await web3Request("cfx_getBlockByEpochNumber", [number, true], web3Instance);
...
export const showTransaction = function(hash) {
  return async function(dispatch, getState) {
    ...
    let transaction = await web3Request("cfx_getTransactionByHash", [hash], web3Instance);
    ...
    for (let j = 0; j < receipt.logs.length; j++) {
      ...
      if (typeof blockTimestamps[log.blockNumber] === "undefined") {
        const block = await web3ActionCreator(dispatch, getState, "cfx_getBlockByEpochNumber", [
          log.blockNumber,
          false,
        ]);
