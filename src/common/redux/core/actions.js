...
export const getGasPrice = function() {
  return async function(dispatch, getState) {
    let gasPrice = await web3ActionCreator(dispatch, getState, "cfx_gasPrice");
...
export const getGasLimit = function() {
  return async function(dispatch, getState) {
    let block = await web3ActionCreator(dispatch, getState, "cfx_epochNumber", [
      //"latest",
    ]);
...
export const getBlockSubscription = function() {
  return async function(dispatch, getState) {
    let subscription = await web3ActionCreator(
      dispatch,
      getState,
      //fixme, 这个地方需要从新写，不支持ws subscribe, 用loogpool
        //这个函数需要重写
      "subscribe",
      ["newBlockHeaders"],
    );
